        ...
        if (DEFIF and "endif" not in fields[0] and "ifdef" not in fields[0] and "//" not in fields[0]):
            if (int(fields[3]) > 32):
                ...
                for i in range(0, numOfRows):
                    fields[2] = "B"+str(i)  # name this as a repeating 8 bit field
                    structure += processColumns(fields, columns)
            else:
                structure += processColumns(fields, columns)

if len(sys.argv) < 5:
    print ('Usage: genSpacecraft <FOXID> <rt|max|min|err|exp|wod|can|canwod> <fileName.csv> <include sections> <ifdef keywords>')
